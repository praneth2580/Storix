...
/* ------------------------
   ENRICH / FK MAPS (cached)
   ------------------------ */
function getFKMaps() {
    const cache = CacheService.getScriptCache();
    const cached = cache.get("FK_MAPS");
    if (cached) {
        try {
    ...
    const readSheet = (name) => {
        ...
        raw.forEach(r => {
            ...
            if (obj.id !== undefined && obj.id !== "") map[String(obj.id)] = obj;
        });
        return map;
    };
    ...
    cache.put("FK_MAPS", JSON.stringify(fk), 60);
    return fk;
}
...
function enrichRow(row, sheetName, fk) {
    ...
    if (sheetName === "Variants") {
        ...
        enriched.productName = product ? product.name : "";
    }
    if (sheetName === "Stock") {
        const variant = fk.variantObjMap[row.variantId];
        const product = variant ? fk.productObjMap[variant.productId] : null;
    ...
    if (sheetName === "Sales") {
        ...
        enriched.customerName = customer ? customer.name : "";
    }
    return enriched;
}
...
function handleGet(e) {
    ...
    let rows = raw.map(r => enrichRow(Object.fromEntries(headers.map((h, i) => [h, r[i]])), sheetName, fk));
    ...
    // minimal listing option
    if (e.parameter.minimal === "true") {
        rows = rows.map(r => ({ id: r.id, name: r.name || r.productName || r.sku, sku: r.sku || null, sellingPrice: r.sellingPrice || null }));
    }
...
function handleCreate(e) {
    ...
    CacheService.getScriptCache().remove("FK_MAPS");
