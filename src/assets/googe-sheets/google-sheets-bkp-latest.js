...
function getFKMaps() {
  ...
  // ---- PRODUCTS ----
  const productSheet = ss.getSheetByName("Products").getDataRange().getValues();
  const productHeaders = productSheet.shift();
  const productMap = {};
  productSheet.forEach(r => {
    const obj = Object.fromEntries(productHeaders.map((h,i) => [h, r[i]]));
    productMap[obj.id] = obj; // store FULL PRODUCT object
  });

  // ---- VARIANTS ----
  const variantSheet = ss.getSheetByName("Variants").getDataRange().getValues();
  const variantHeaders = variantSheet.shift();
  const variantMap = {};
  variantSheet.forEach(r => {
    const obj = Object.fromEntries(variantHeaders.map((h,i) => [h, r[i]]));
    variantMap[obj.id] = obj; // store FULL VARIANT object
  });
...
function enrichRow(row, sheetName, fk) {
  ...
  if (sheetName === "Stock") {
    const variant = fk.variantMap[row.variantId];
    if (variant) {
      enriched.variant = variant;          // whole variant object
      enriched.variantSku = variant.sku;

      const product = fk.productMap[variant.productId];
      if (product) {
        enriched.product = product;        // whole product object
