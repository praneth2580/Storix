          ...
          <table className="w-full text-left border-collapse min-w-[800px]">
            ...
            <tbody className="divide-y divide-border-primary">
              {filteredProducts.map(product => <tr key={product.id} className="hover:bg-tertiary transition-colors group">
                  ...
                  <td className="p-3 text-right font-mono text-sm">
                    <span className={product.stock < 10 ? 'text-accent-red' : 'text-text-primary'}>
                      {product.stock}
                    </span>
                  </td>
                  <td className="p-3 text-center">
                    <span className={`text-[10px] px-2 py-0.5 rounded border ${product.status === 'Active' ? 'bg-accent-green/10 text-accent-green border-accent-green/20' : product.status === 'Archived' ? 'bg-tertiary text-text-muted border-border-secondary' : 'bg-accent-amber/10 text-accent-amber border-accent-amber/20'}`}>
          ...
          <div className="bg-secondary border border-border-primary w-full max-w-2xl shadow-2xl animate-in fade-in zoom-in duration-200 rounded-lg overflow-hidden">
            ...
            <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6 max-h-[80vh] overflow-y-auto">
              ...
              <div className="space-y-1">
                ...
                <input type="text" defaultValue={editingProduct?.sku} className="w-full bg-primary border border-border-primary p-2 text-sm text-text-primary font-mono focus:border-accent-blue focus:outline-none rounded-sm" placeholder="e.g. ELEC-001" />
              </div>
              ...
              <div className="space-y-1">
                ...
                <input type="number" defaultValue={editingProduct?.price} className="w-full bg-primary border border-border-primary p-2 text-sm text-text-primary font-mono focus:border-accent-blue focus:outline-none rounded-sm" placeholder="0.00" />
              </div>
              ...
              <div className="space-y-1">
                ...
                <input type="number" defaultValue={editingProduct?.stock} className="w-full bg-primary border border-border-primary p-2 text-sm text-text-primary font-mono focus:border-accent-blue focus:outline-none rounded-sm" placeholder="0" />
              </div>
            </div>
